...
var HelpPages = []HelpPage{
	{
		...
		Items: [][2]string{
			...
			{"/", "Open Slash Menu (File, Home, Insert, Formulas, Data, View, Chart, Help)"},
			...
			{"Ctrl+H", "Find and Replace text/number/formula (/HE)"},
	...
	},
	{
		...
		Items: [][2]string{
			...
			{"/H (Home)", "Undo/Redo/Cut/Copy/Paste (/HU /HR /HX /HC /HV), Paste-Special (/HS), Clear (/HK), Find (/HF /HN /HB /HE /HA), Goto (/HG), Number (/HM), Align (/HL), Cells (/HW)"},
			{"/I (Insert)", "Function (/IF), Rows (/IR), Columns (/IC), Today (/IT), Now (/IN), Chart (/IH)"},
			{"/O (Formulas)", "AutoSum (/OS), Insert-Function (/OF), Average/Count/Max/Min (/OA /OC /OM /OI), Recalculate (/O9, F9)"},
			{"/D (Data)", "Sort (/DS), AutoFill (/DA), Fill (/DF), Transpose (/DT), Names (/DN → C/D/L)"},
			{"/V (View)", "Freeze-Panes (/VF), Select-Sheet (/VS), Next/Prev (/VN /VP), Add/Delete/Rename (/VA /VD /VR)"},
			{"/C (Chart)", "View (/CV, F10), Type (/CT), Title (/CI), X-Axis (/CX), Series A-F (/CA../CF), Status (/CS), Save-PNG (/CP)"},
