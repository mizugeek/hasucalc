...
1. [プロジェクト概要 & 設計思想](#1-プロジェクト概要--設計思想)
   - [1.4 仕様の基本方針と動作保証範囲](#14-仕様の基本方針と動作保証範囲)
...
3. [機能仕様詳細](#3-機能仕様詳細)
   ...
   - [3.13 ヘッドレスCLI & AIエージェント連携仕様](#313-ヘッドレスcli--aiエージェント連携仕様)
...
**設計の独立性とデータ連携**: HasuCalc は独自のデータモデルとネイティブ形式（`.hwk` / `.hwkz`）を正本とする独立したターミナル表計算ソフトウェアです。`.xlsx` や `.ods` などの外部形式サポートは表データの相互運用のためのブリッジ機能であり、他ソフトウェアの再現やクローンを目的とするものではありません。
...
HasuCalc では、本書に記述されたアーキテクチャおよび動作仕様を製品の正本とします。一般的なスプレッドシート記法とクラシックな構文の双方向をサポートしつつ、ターミナル環境に最適化された独自モデルを保持します。

#### 1. 動作保証の対象（コア機能）

以下の項目は HasuCalc のコア機能として動作を保証します：
* **疎行列グリッド**: 最大 1,048,576行 × 16,384列（`A`〜`XFD`）をサポートし、使用セルのみメモリを消費。
* **セル型とプレフィックス**: `NUMBER`, `LABEL`（`'` 左寄せ、`"` 右寄せ、`^` 中央）、`FORMULA`、`BOOLEAN`、`EMPTY` の完全保持（§3.1）。
* **計算エンジン**: 構文解析・評価、循環参照の自動検知（`CIRCULAR REF`）、AUTO モードでの自動再計算。
* **参照の自動追従**: 行・列の挿入／削除、切り取り／コピー／貼り付け、シートの追加・削除・改名に伴うセル参照の自動追従（無効参照は `#REF!`）。
* **ネイティブ形式の完全性**: `.hwk` / `.hwkz` の入出力において、セル値・数式・名前・グラフ設定・再計算モードを完全に保持。
* **外部表データ連携**: `.xlsx` / `.ods` との入出力において、表データ（セル・数式・名前付き範囲・固定枠・再計算モード）を正確に保持（グラフ等の付随オブジェクトは対象外。§3.9）。
* **セル関数**: `ROW()` / `COLUMN()`（引数なし）は当該セル自身の行／列番号を返却。

#### 2. 数式・構文体系

使い慣れた標準構文と、伝統的なクラシック構文の両方を受け付けます。

| 構文要素 | 標準記法 | クラシック互換記法 | 備考 |
|---|---|---|---|
| 数式プレフィックス | `=` (例: `=SUM(A1:B10)`) | `@` (例: `@SUM(A1..B10)`), `+` (例: `+A1+B1`) | いずれも数式として受理 |
| 範囲指定子 | `:` (例: `A1:B10`) | `..` (例: `A1..B10`) | 全列指定 `A:A` は used range まで走査 |
| 論理演算子 | 関数 `AND()`, `OR()`, `NOT()` | インライン `#AND#`, `#OR#`, `#NOT#` | 数式内で直接使用可能 |
| 主要関数の別名 | 正本名（`AVERAGE`, `LEN`, `REPT`, `PMT`, `PRODUCT`, `STDEV.P`） | エイリアス（`AVG`, `LENGTH`, `REPEAT`, `PAYMT`, `MULTIPLY`, `STD`, `STRING`） | エイリアスも同等に評価 |
| 指数書式 | `(E2)` (例: `1.23E+04`) | `(S2)` | 同等に処理 |
| 演算子の優先度 | 単項マイナスは `^` より優先 | — | `-2^2` は `(-2)^2` = `4` と評価（`-4` には `-(2^2)` と指定） |

#### 3. エラー体系

HasuCalc ではエラー表示を以下の4種類に集約して明確化しています：

| エラー表示 | 意味と発生条件 |
|---|---|
| `ERR` | 一般的な計算エラー（ゼロ除算、引数の型不正、ドメインエラーなど）。関数 `ERR()` でも明示的に返却可能。 |
| `NA` | 検索該当なし・未定義値（`VLOOKUP` や `MATCH` 等での未発見、関数 `NA()` など）。 |
| `CIRCULAR REF` | セル間の循環参照を検出した場合。 |
| `#REF!` | 行列・シートの削除により無効となったセル参照。 |

※ `FIND` / `SEARCH` で部分文字列が見つからない場合は `NA` を返し、`ISERR` は `NA` を除外して判定します。

#### 4. HasuCalc の独自仕様

* **保存形式**: Git や AI で扱いやすい非圧縮コンパクト JSON（`.hwk`）および透過 GZIP（`.hwkz`）。
* **グラフシステム**: 1シートにつき1つのグラフ設定（系列 A〜F）。端末内プレビューと 1280×720 HD PNG 出力に対応（§3.9）。
* **通貨表示**: `$` に限定されず、セルごとに任意の通貨文字列（`¥`, `€`, `£`, `USD ` など）を前置可能（§3.1）。
* **パーセント指定**: 数値リテラル直後の `%`（例: `50%` → 0.5）のみを受け付けます（`=A1%` は非対応）。
* **マークアップ取り込み**: Markdown（`.md`）および HTML（`.html`）から表をグリッドに展開し、本文をラベルとして取り込む利便機能を提供（§3.10）。

#### 5. サポート対象外（設計上のスコープ外）

ターミナル表計算としての軽快性とシンプルさを最優先するため、以下の機能は設計上サポート対象外としています：
* マクロ／スクリプト言語実行（VBA 等）
* ピボットテーブル、スライサー、条件付き書式、結合セル、コメント、入力規則
* 動的配列数式・スピル
* 外部形式（`.xlsx` / `.ods`）のチャート・図形・マクロ等の非表データオブジェクト
...
標準的な `=` 記法に加え、クラシックな `@` 記法や `+` で始まる数式など、多様な入力形式を受け付けます。優先順位、エラー表示、およびエイリアスの詳細は [§1.4](#14-仕様の基本方針と動作保証範囲) に従います。

* **数式プレフィックス**: `=`（標準数式記法）、`@`（関数記法）、`+`（クラシック数式記法）。
* **範囲指定子**: コロン `:`（例: `A1:B10`）およびピリオド2つ `..`（例: `A1..B10`）。
* **べき乗と単項マイナス**: 単項マイナスは `^` より優先されます（`-2^2` = `(-2)^2` = `4`。`-4` を計算したい場合は `-(2^2)` と記述）。
* **対応関数（70種以上）**:
...
機能別に体系化された直感的な階層型メニューです。`Ctrl+K` コマンドパレットからも同様の機能にアクセスできます。
...
* **外部スプレッドシート形式（`.xlsx` / `.ods`）との連携方針**: HasuCalc のグラフは端末描画および高解像度 PNG 出力に特化した独自モデル（1シート1グラフ・系列 A〜F）を採用しています。このため外部形式との連携ではセル・数式・名前などの表データのみを入出力し、チャート要素は含めません。グラフの共有や持ち出しには PNG エクスポートを使用します。
...
HasuCalc の正本形式は `.hwk` / `.hwkz` です。以下の外部フォーマットは、**外部環境との表データ相互運用**を目的としてサポートされています。

* **Excel ワークブック (`.xlsx`)**: 複数シートのセルデータ、数式、名前付き範囲などの表データを入出力（グラフは対象外。§3.9参照）。
* **LibreOffice Calc (`.ods`)**: 複数シートの表データを入出力（グラフは対象外。§3.9参照）。
...
* **Open ダイアログ**（`Ctrl+O` / `/FO` / パレット「Open File」）: ディレクトリに加え、次の拡張子のファイルを一覧表示する。
  * ネイティブ: `.hwk`, `.hwkz`, `.hwk.gz`, `.json`
  * Office / テキスト橋渡し: `.xlsx`, `.xlsm`, `.ods`, `.ots`, `.csv`, `.tsv`
  * マークアップ橋渡し: **`.md`**, **`.markdown`**, **`.html`**, **`.htm`**
  * レガシー: `.wk3`, `.123`（および該当する場合 `.123.json`）
* `.md` / `.html` / `.htm` を選択するとマークアップ取り込み（§3.10）を実行する。CSV / XLSX / ODS / HWK は従来どおりのローダを使う。
...
HasuCalc 2.0 では、`engine_test.go` / `mega_test.go` / `bugfix_regression_test.go` などの回帰テストスイートによって品質を保証しています。テストが検証するのは [§1.4](#14-仕様の基本方針と動作保証範囲) に定義された仕様と契約です。

1. **数式 & 関数評価**: 主要関数・エラー系・シート間参照。
2. **再計算 & 循環参照**: 依存順の stale 回避、`CIRCULAR REF` 検知、全列参照の used-range 制限。
3. **ファイルI/O**: Compact `.hwk`, GZIP `.hwkz`, Excel `.xlsx`, ODS, CSV（UTF-8 BOM）、Markdown/HTML 読み込み（表＋本文ラベル）。
4. **PNG画像エクスポート**: 出力生成とシグネチャ確認。
5. **UI**: ファイルピッカー編集（Open での `.md` / `.html` 一覧を含む）、マウスドラッグ選択、ホイール。
