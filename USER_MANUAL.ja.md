...
関数一覧ブラウザは **`/OF`**（Formula → Function）またはパレットから。関数の説明は本節を日本語で記載しています。
...
| `/EFF` `/EFE` `/EG` | 検索 / 置換 / ジャンプ |
| `/MNC` … | 数値書式（通貨・% など） |
| `/RI` `/RD` | 行の挿入 / 削除 |
| `/LI` `/LD` `/LWS` | 列の挿入 / 削除 / 幅設定 |
| `/WA` `/WD` `/WR` | シートの追加 / 削除 / 改名 |
| `/OF` | 関数ブラウザ |
| `/OAS` / `/O9` | Aggregate Sum / 再計算 |
| `/DS` `/DFA` `/DFS` `/DT` | ソート / AutoFill / 連続入力 / 転置 |
| `/WF` | 枠の固定 |
...
| **`.md` / `.html`** | **読み込み**: 本文→`Document`、表はすべて `Tables` 1枚に縦積み（バナー＋空行で区切り）。Markdown は表として**書き出し**も可。 |
